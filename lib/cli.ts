...
async function main() {
    ...
    const project = (await client.projects.list()).find(p => projectParam === p.name);
    if (project) {
        const device = (await client.devices.list(project)).find(d => deviceParam === d.serialNr);
        if (device) {
            const values = await client.values.list(project, device);
            const knownValues = values.filter(
                value =>
                    mapper.mapGridVisToProtoBuffer(value.valueType.value) &&
                    mapper.mapSecondsToProtoBuffer(value.timebase),
            );
